...
export default function PostMetaInfo({author, categories, commentCount}: PostMetaInfoProps) {
	const { name } = author?.node || { name: 'Sem autor' }

	return (
		<div className="flex items-center gap-8">
			<div className="flex flex-col justify-center gap-2">
				<span className="text-[10px] uppercase font-bold text-gray-800 leading-tight">Autor</span>
				<span className="font-medium uppercase text-gray-400">{name}</span>
			</div>
			<div className="flex flex-col justify-center gap-2">
